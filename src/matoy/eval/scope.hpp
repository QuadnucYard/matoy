...
class Scope {
  public:
    ...
    using storage_type = std::unordered_map<std::string, value_type>;
    ...
    auto get(std::string_view var) const -> std::optional<value_const_pointer> {
        // FIXME view
        if (auto it = values.find(key_type{var}); it != values.end()) {
            return &it->second;
        }
        return std::nullopt;
    }

    auto get(std::string_view var) -> std::optional<value_pointer> {
        // FIXME view
        if (auto it = values.find(key_type{var}); it != values.end()) {
            return &it->second;
        }
...
struct Scopes {
    ...
    auto get(std::string_view var) -> diag::HintedResult<Scope::value_pointer> {
        using namespace matoy::utils;
        if (auto ret = ranges::find_map(scopes | std::views::reverse, [var](Scope& scope) { return scope.get(var); })) {
            return *ret;
        } else {
            return std::unexpected{diag::Hints{std::format("unknown variable: {}", var)}};
        }
